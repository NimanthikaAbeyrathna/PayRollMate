...
/*////////////////////////////////////////////*/
exports.router.get("/bank", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const banks = yield pool.query('SELECT * FROM bank');
        const formattedBanks = banks.map((row) => {
            return {
                id: row.id,
...
exports.router.post("/bank", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const bankData = req.body;
    if (!bankData) {
        res.sendStatus(400);
        return;
    }
    const result = yield pool.query('INSERT INTO bank (bankName) VALUES (?)', [bankData.bankName]);
    const bank = {
        id: result.insertId,
...
exports.router.delete("/bank/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield pool.query('DELETE  FROM  bank WHERE  id = ?', [req.params.id]);
    res.sendStatus(result.affectedRows ? 204 : 404);
}));
/*////////////////////////////////////////////*/
...
exports.router.post('/printbanksalarysheet', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const printRequest = req.body;
    const requestBank = printRequest[0];
    const requestYear = printRequest[1];
    const requestMonth = printRequest[2];
    const employeeList = yield pool.query('SELECT * FROM employee WHERE bank_name=?', [requestBank]);
    const salaryListOfRelavantBank = [];
    for (const employeeListElement of employeeList) {
        const salary = yield pool.query('SELECT * FROM salary WHERE employeeid=? AND year=? AND month=?', [employeeListElement.employee_id, requestYear, requestMonth]);
        salaryListOfRelavantBank.push(salary[0]);
    }
    const formattedSalaryList = salaryListOfRelavantBank.map((row) => {
        return {
            id: row.id,
            employeeID: row.employeeid,
            year: row.year,
